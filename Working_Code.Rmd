...
Our first example of using `leaflet()` is to map out the best city in the US: Seattle! As we learned recently, we will use the pipe operator %>% in the `magrittr` package in order to write our program most effectively. While this may not make much sense at the moment, we will explain what both `addTiles()` and `addMarkers()` means later. For now, you just need to know that we are building the images of the map through `addTiles()` and specifying which city is the best city in the US thru its longitude and latitude coordinates and labeling it with `addMarkers()`.
```{r}
seattle <- leaflet() %>%
...
The `leaflet()` function returns a Leaflet map widget or interface, which stores a list of objects that can be modified or updated later. The map widget displays an interactive map control.
...
`pal` is a function that we ourselves will write, which takes a variable and maps that variable to a specific gradient of colour. So say we were plotting Life Expectancy. `pal` would take Life Expectancy as its argument and give each value of Life Expectancy for every country a specific colour.
...
binpal <- colorBin("Blues", countries$gdp_md_est, bins = breaks, pretty = FALSE)
```
...
Rectangles are a little bit more clunky. As they're rectangles, they need four different long-lat values: two long (lng1 and lng2) and two lat (lat1 and lat2). Beyond that, they're not that difficult, or useful for that matter, in my opinion. So we won't spend too much time on them.
...
All of you should be familiar with `Spatial Polygons` and `SpatialPolygonsDataFrames` (spdf) by now. The former is a function that has long lat values of a polygon (these long lat values can change based on the CRS chosen, as we've already seen in class), whilst the latter are just a data frame of several `Spatial Polygons`.
...
Yup, we're still not done with Polygons. Quiet, stop whining. Labels are cute and not as complicated. Say you wanted to get information about a specific polygon (or the country it represents) whenever you hovered above it. That's what labels do. For labels to be effective, you're going to have to know some basic sprintf. For the exercise there are four things you're going to have to know. You'd want your labels to take in not just "Nevada", but any name you give it. This is done with the replacement that sprintf offers. Say you wanted to input muptiple strings without writing them. In that case you'd give `sprintf()` the following: quotes that look like this "%s" and a string or vector of strings that ought to replace it.
...
Don't worry too much about the specifics. This is a bit of HTML you're learning right now. You'll need it for the exercise, but don't worry about actually understanding it.
