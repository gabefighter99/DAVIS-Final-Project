...
The `addPolygons()` function takes several arguments. Among those, the ones of note are `fillColor`, `label` and `highlightOptions`. The first is fairly self-explanatory. We've done this several times in the past. There is one important and a little confusing difference however. If we wanted the polygons' colour to be the same, then that's simple, we just say `fillColor = "green"`. However, that's boring and competely useless if we have many polygons on the map, especially if we want the colour to represent something. `addPolygons()`'s `fillColor` in that case, takes an argument called `pal`. This argument is so important, I'm going to dedicate a section to it.
...
Most of the time in this class however, we've been using discrete colours for a continuous input. 
